...
if __name__ == '__main__':
    ...
    # 5 模型评估
    # 支持向量机回归模型评估
    print("支持向量机回归的默认评估值为：", rfr.score(x_test, y_test))
    print("支持向量机回归的R_squared值为：", r2_score(y_test, rfr_y_predict))
    print("支持向量机回归的均方误差为:", mean_squared_error(ss_y.inverse_transform(y_test),
                                              ss_y.inverse_transform(rfr_y_predict)))
    print("支持向量机回归的平均绝对误差为:", mean_absolute_error(ss_y.inverse_transform(y_test),
                                                 ss_y.inverse_transform(rfr_y_predict)))
